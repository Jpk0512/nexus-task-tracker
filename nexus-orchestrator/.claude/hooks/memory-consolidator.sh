...
DB="$REPO_ROOT/.memory/project.db"
...
# Last 50 context_log rows
rows = conn.execute("""
    SELECT action_type, summary, logged_at
    FROM context_log
    ORDER BY logged_at DESC LIMIT 50
""").fetchall()
log_lines = [f"[{r['logged_at']}] {r['action_type']}: {r['summary'] or ''}" for r in rows]

# Recent validation_log
val_rows = conn.execute("""
    SELECT task_id_or_hash, verdict, logged_at
    FROM validation_log
    ORDER BY logged_at DESC LIMIT 20
""").fetchall()
val_lines = [f"  {r['task_id_or_hash']}: {r['verdict']} @ {r['logged_at']}" for r in val_rows]
...
sess_row = conn.execute("""
    SELECT summary, next_step, ended_at
    FROM sessions
...
last_session = ""
if sess_row:
    last_session = f"summary: {sess_row['summary'] or 'n/a'}\nnext_step: {sess_row['next_step'] or 'n/a'}"

# Recent decisions
dec_rows = conn.execute("""
    SELECT id, title, decision, logged_at
    FROM decisions
    ORDER BY logged_at DESC LIMIT 5
""").fetchall()
dec_lines = [f"  {r['id']} [{r['logged_at']}]: {r['title']} → {r['decision']}" for r in dec_rows]
...
try:
    ...
    val_rows = conn.execute("""
        SELECT task_id_or_hash, verdict, logged_at
        FROM validation_log ORDER BY logged_at DESC LIMIT 20
    """).fetchall()
    val_lines = [f"  {r['task_id_or_hash']}: {r['verdict']} @ {r['logged_at']}"
                 for r in val_rows]

    task_rows = conn.execute("""
        SELECT id, title, status, assigned_to
        FROM tasks WHERE status IN ('pending','in_progress') ORDER BY id
    """).fetchall()
    task_lines = [f"  {r['id']} [{r['status']}] {r['title']} → {r['assigned_to'] or 'unassigned'}"
                  for r in task_rows]
    ...
    dec_rows = conn.execute("""
        SELECT id, title, decision, logged_at FROM decisions
        ORDER BY logged_at DESC LIMIT 5
    """).fetchall()
    dec_lines = [f"  {r['id']} [{r['logged_at']}]: {r['title']} → {(r['decision'] or '')[:100]}"
                 for r in dec_rows]
...
" "$CONTEXT" "$SYSTEM_PROMPT" 2>/dev/null)
