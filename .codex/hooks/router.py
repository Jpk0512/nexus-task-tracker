...
# OPT-012: when the OpenAI-compatible endpoint returns logprobs, router_core derives
# a top-2 probability margin that is far better calibrated than the verbalized number
# on a small local model. When a margin IS present we gate on it instead of the
# self-reported confidence; this floor stays deliberately conservative (the priority
# is CAPTURING the margin for the OPT-007 eval/fine-tune harness, not re-tuning
# routing). When no margin is available (server omits logprobs) we fall back to the
# verbalized-confidence gate unchanged. Override: _HOOK_THRESHOLD_MARGIN=0.20
THRESHOLD_MARGIN = float(os.environ.get("_HOOK_THRESHOLD_MARGIN", "0.10"))
...
# Mirrors broker.router_train.transcript.is_genuine_user_prompt exactly.
# Injected Claude Code machinery messages (task-notifications, system-reminders,
# hook context blobs) are NOT real developer routing queries.  When the incoming
# UserPromptSubmit payload is NOT genuine we skip persisting to router_decisions.jsonl
# so the file stays free of noise that would corrupt the (session_id, prompt_hash)
# join key used by dispatch-capture and completion-capture.
...
def _is_genuine_user_prompt(text: str) -> bool:
    ...
    Standalone copy of broker.router_train.transcript.is_genuine_user_prompt —
    hooks run under ambient python3 (3.9) without access to the broker .venv,
    so duplication across the hook boundary is intentional.
    """
    stripped = text.strip()
...
def main() -> None:
    ...
    # BUG #2 fix: skip non-genuine turns (task-notifications, system-reminders,
    # hook context blobs).  These polluted router_decisions.jsonl and corrupted the
    # (session_id, prompt_hash) join key used by dispatch-capture/completion-capture.
    # Fail-open: if the check itself raises, fall through to normal routing.
    try:
        if not _is_genuine_user_prompt(user_msg):
            sys.exit(0)
    except Exception:
        pass
    ...
    def _decision_record(decision: str, model_out: dict | None) -> dict:
        rec = {
            ...
            # OPT-012: logprob-derived top-2 margin (None when the server omitted
            # logprobs). Recorded on EVERY row — including fallthroughs — so the
    ...
    # OPT-012 gate: when a logprob margin is present, gate on it (better calibrated
    # than the verbalized number on a small local model); otherwise fall back to the
    # verbalized-confidence gate unchanged. This is intentionally a light touch — the
    # priority is capturing the margin, not re-tuning routing behavior.
    if confidence_margin is not None:
        gate_pass = confidence_margin >= THRESHOLD_MARGIN
    else:
        gate_pass = classification is not None and classification.get("confidence", 0.0) >= THRESHOLD_LLM

    if classification is None or not gate_pass:
    ...
    # Guard: empty/whitespace persona is semantically invalid — abstain rather than
    # emit a broken chip with persona="".  Mirror the fallthrough pattern (capture
    # the decision, exit 0, no additionalContext output).
    if not persona or not persona.strip():
        _write_decision("fallthrough", classification)
        sys.exit(0)
