...
- T1 light: emit `"lens_tier": "T1"` in output JSON. Run deterministic + light semantic. DB row with `agent_validated='lens'`, real verdict.
...
- Lens does NOT pass a UI fix without an `aside` before+after screenshot
  in the implementer's response (gate enforced by `visual-evidence-gate.sh`; accountable-skip via `verification_result.visual_skip_reason`).
