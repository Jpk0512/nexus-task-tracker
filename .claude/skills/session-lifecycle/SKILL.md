...
at the close of a session. Five commands: `task-start`, `task-done`, `decision`,
`snapshot`, `session-end` — full command text + when-to-use-which table:
**`references/logging-commands.md`**.

**The one rule that matters most:** the Stop hook only writes a context snapshot; it
never closes the session. `session-end` is the canonical (and only) way to close one —
skipping it leaves the session open with no comparison baseline for the next session's
staleness check.

A full worked session (start → mid-session snapshot → task-done → session-end):
`examples/session-start-to-end.md`.

## References

- `references/logging-commands.md` — the five logging commands in full, plus a
  when-to-use-which table.
- `examples/session-start-to-end.md` — a complete session walkthrough from `context dump`
  to `session end`.
