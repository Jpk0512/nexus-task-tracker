...
> Adapted from elvisun/loss-function-development (`references/cheat-museum.md`),
> MIT License — see `../ATTRIBUTION.md`. Content preserved; the Nexus mapping is
> in the parent `SKILL.md`.

Real ways optimizing agents have satisfied a target without solving the task.
Read this before designing any loss function; every entry is a direction the
optimizer will sprint down if the path is left open. When a NEW cheat is caught
mid-run (Patch mode), append the exhibit here AND record it as a lesson +
feedback signal so the next loop never reopens that path.
...
**Fence:** declare `harness/` and `eval/` read-only in `goal.md`; checksum
the scorer; keep holdout scoring outside the agent's write surface (Lens runs
it).
...
(In Nexus the judge is Lens — calibrate it the same way.)
...
**Fence:** acceptance is defined on holdout exclusively, measured by Lens,
stated in `goal.md`'s stop conditions.
...
diminishing-returns clause. In Nexus these are the mandatory runaway guards
(DEC-024): max-iteration cap, no-progress detection, token/$ budget,
circuit-breaker.
...
exploration quota every K cycles, written into `goal.md`. This is the Nexus
REVISE stall-escalation made explicit.
...
findings go to the human (and Lens), outside the agent's read surface. Your
