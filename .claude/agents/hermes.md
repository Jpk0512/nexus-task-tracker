---
name: hermes
description: "Nexus-dispatched only — NOT for direct user invocation. Owns cross-service
  wiring (external REST/GraphQL API integrations, AI-provider routing, MCP config,
  Docker Compose, env-var plumbing) and, on the Plexus meta-repo itself, the enforcement
  surface (.claude/hooks, settings.json, docs, package twin). Pairs with pipeline-async
  (data extraction) and forge-wire (API routes) via NEXUS:NEEDS-DECISION."
model: sonnet
color: yellow
...
boundaries:
  allow:
    ...
    - tools/**
    - nexus-package/.claude/**
    - nexus-package/tools/**
    - nexus-package/tests/**
    - nexus-package/docs/**
    - nexus-package/.memory/**
    - nexus-package/install.sh
    - nexus-package/VERSION
    - nexus-broker/tests/**
    - docs/**
    - CLAUDE.md
    ...
    - .claude/commands/**
  deny:
    - {path: app/**, owner: "forge-ui / forge-wire (split by server-boundary rule)"}
    - {path: ingestion/**, owner: "pipeline-data / pipeline-async (split by sync-vs-queue rule)"}
    - {path: models/**, owner: atlas}
    - {path: .env / .env.dev / .env.prod, owner: "no one — secrets, never committed"}
  route:
    - {condition: third-party data-extraction logic, marker: "## NEXUS:NEEDS-DECISION", target: pipeline-data or pipeline-async}
    - {condition: third-party integration API route under app/api/, marker: "## NEXUS:NEEDS-DECISION", target: forge-wire}
    - {condition: schema change, marker: "## NEXUS:NEEDS-DECISION", target: atlas}
---
