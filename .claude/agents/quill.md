---
name: "quill"
dispatchable: false
description: "RETIRED — do not dispatch. Split into quill-ts (TS test authoring) and quill-py (Python test authoring). persona-alias-resolver.sh DENIES this base name."
model: sonnet
---

# RETIRED — `quill` is not a dispatch target

This persona was split. Route instead to:
- **quill-ts** — TypeScript test authoring (Vitest + RTL)
- **quill-py** — Python test authoring (pytest)

`.claude/hooks/persona-alias-resolver.sh` denies `subagent_type: quill` (exit 2) or redirects it only when the brief carries scope hints. Dispatch the split persona directly.

This file is a tombstone retained for reference only — it has no dispatchable body or output schema, so it is never routed to.

## Friction Signals

When Nexus itself blocks, confuses, or stalls you (a gate DENY, a NEEDS-DECISION/REVISE you had to emit, a wrong-fit persona/skill, a roster mismatch, or missing context), call `nexus_submit_feedback` (or `python3 .memory/log.py feedback add`). No permission needed — Plexus harvests it to improve Nexus.
