...
def _assert_nested_deny(result: subprocess.CompletedProcess[str]) -> str:
    ...
    # _gate_deny.deny() prepends "[GATE:<code>] " so the full string is
    # "[GATE:BROKER/DISPATCH-BLOCKED] Task dispatch blocked: <detail>".
    # The substantive content is unchanged; use `in` to tolerate the prefix.
    assert "Task dispatch blocked: " in reason, (
        f"Reason must contain 'Task dispatch blocked: ', got: {reason!r}"
    )
