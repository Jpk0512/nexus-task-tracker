...
export const getTasks = async ({
	...
	if (input.search) {
		...
		if (isUUID) {
			...
			// websearch_to_tsquery tolerates arbitrary user input (no need to
			// escape & | ! ( ) : * etc.) and is the right tool for search bars.
			// to_tsquery would throw on `hello` because it expects strict
			// lexeme syntax like `hello:*`.
			whereClause.push(
				sql`(websearch_to_tsquery('english', unaccent(${input.search})) @@ ${tasks.fts})`,
			);
		}
	}
