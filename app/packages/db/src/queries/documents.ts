...
export const getDocuments = async ({
	...
	if (projectId !== undefined && projectId !== null) {
		whereClause.push(eq(documents.projectId, projectId));
	}
	...
	if (search) {
		// websearch_to_tsquery tolerates arbitrary user input; to_tsquery
		// throws on plain words like `hello` which lack lexeme syntax.
		whereClause.push(
			sql`(websearch_to_tsquery('english', unaccent(${search})) @@ ${documents.fts})`,
		);
	}
