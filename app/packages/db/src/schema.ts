...
export const projects = pgTable(
	"projects",
	{
		...
		description: text("description"),
		color: text("color"),
		// Short uppercase prefix used to render task IDs as PFX-123 in the UI.
...
export const documents = pgTable(
	"documents",
	{
		...
		content: text("content"),
		teamId: text("team_id").notNull(),
		// Optional pin to a specific project. Null = team-wide doc.
		projectId: text("project_id"),
...
// ── Todos ────────────────────────────────────────────────────────────────
//
// Quick checklist captures, distinct from project tasks. No status pipeline,
// no due date, no priority — just check it off when it's done. Optional
// project link, free-form tags, inline attachments (notes or doc links).
...
export const todos = pgTable(
	...
	(table) => [
		index("todos_team_id_index").on(table.teamId),
		index("todos_checked_index").on(table.checked),
		index("todos_order_index").on(table.order),
		foreignKey({
			columns: [table.teamId],
			foreignColumns: [teams.id],
			name: "todos_team_id_fkey",
		}).onDelete("cascade"),
		foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const todoAttachmentKindEnum = pgEnum("todo_attachment_kind", [
	"note",
	"doc_link",
]);
...
export const todoAttachments = pgTable(
	"todo_attachments",
	{
		...
		kind: todoAttachmentKindEnum("kind").notNull(),
		title: text("title").notNull(),
		// Inline markdown when kind=note. Empty when kind=doc_link.
		content: text("content"),
		// FK when kind=doc_link.
		docId: text("doc_id"),
		order: integer("order").default(0).notNull(),
		createdAt: timestamp("created_at", {
			withTimezone: true,
	...
	(table) => [
		index("todo_attachments_todo_id_index").on(table.todoId),
		foreignKey({
		...
		foreignKey({
			columns: [table.docId],
			foreignColumns: [documents.id],
			name: "todo_attachments_doc_id_fkey",
		}).onDelete("set null"),
	],
);
...
export const librarySources = pgTable(
	"library_sources",
	{
		...
		// Absolute path on host (validated server-side against LIBRARY_ALLOWED_ROOT).
		rootPath: text("root_path").notNull(),
		// "skill" | "agent" | "orchestration" | null (auto-classify).
		kindHint: text("kind_hint"),
		globInclude: text("glob_include").default("**/*.md").notNull(),
		globExclude: text("glob_exclude")
	...
	(table) => [
		unique("unique_library_source_label_per_team").on(
			table.label,
			table.teamId,
		),
		index("library_sources_team_id_index").on(table.teamId),
		foreignKey({
			columns: [table.teamId],
			foreignColumns: [teams.id],
			name: "library_sources_team_id_fkey",
		}).onDelete("cascade"),
	],
);
...
export const libraryEntries = pgTable(
	...
	(table) => [
		unique("unique_library_entry_per_source").on(
			table.sourceId,
			table.relativePath,
		),
		index("library_entries_source_id_index").on(table.sourceId),
		index("library_entries_kind_index").on(table.kind),
		foreignKey({
			columns: [table.sourceId],
			foreignColumns: [librarySources.id],
			name: "library_entries_source_id_fkey",
		}).onDelete("cascade"),
		foreignKey({
			columns: [table.lastEditedBy],
			foreignColumns: [users.id],
			name: "library_entries_last_edited_by_fkey",
		}).onDelete("set null"),
	],
);
...
export const libraryEntryTags = pgTable(
	...
	(table) => [
		primaryKey({
			columns: [table.entryId, table.tag],
			name: "library_entry_tags_pkey",
		}),
		index("library_entry_tags_tag_index").on(table.tag),
		foreignKey({
			columns: [table.entryId],
			foreignColumns: [libraryEntries.id],
			name: "library_entry_tags_entry_id_fkey",
		}).onDelete("cascade"),
	],
);

export const libraryEntryProjects = pgTable(
	"library_entry_projects",
	{
		entryId: text("entry_id").notNull(),
		projectId: text("project_id").notNull(),
		note: text("note"),
	...
	(table) => [
		primaryKey({
			columns: [table.entryId, table.projectId],
			name: "library_entry_projects_pkey",
		}),
		index("library_entry_projects_project_id_index").on(table.projectId),
		foreignKey({
			columns: [table.entryId],
			foreignColumns: [libraryEntries.id],
			name: "library_entry_projects_entry_id_fkey",
		}).onDelete("cascade"),
		foreignKey({
			columns: [table.projectId],
			foreignColumns: [projects.id],
...
// ── Global search view ────────────────────────────────────────────────────
//
// MUST be declared after every table it UNIONs against (documents, todos,
