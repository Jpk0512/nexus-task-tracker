...
describe("FEAT-001 P3 — auth.ts Bearer-token implementation", () => {
	test("auth.ts reads NEXUS_API_TOKEN from process.env (token-check present)", () => {
	...
	test("auth.ts validates Authorization: Bearer <token> header (bearer prefix parsed)", () => {
		const src = readSource(AUTH_MIDDLEWARE_PATH);
		...
		const hasBearerCheck =
			/[Bb]earer/i.test(src) && /[Aa]uthorization/i.test(src);
		expect(hasBearerCheck, "auth.ts must parse the Bearer token header").toBe(
			true,
		);
	});

	test("auth.ts NEXUS_LOCAL_DEV bypass removed (replaced by token gate in P3)", () => {
		const src = readSource(AUTH_MIDDLEWARE_PATH);
	...
	test("auth.ts: token comparison against NEXUS_API_TOKEN rejects wrong tokens (invalid-token 401 path present)", () => {
		const src = readSource(AUTH_MIDDLEWARE_PATH);
		...
		const hasTokenComparison =
			...
			(/!==\s*process\.env\.NEXUS_API_TOKEN|process\.env\.NEXUS_API_TOKEN\s*!==|!==\s*token|token\s*!==/.test(
				src,
			) ||
...
describe("FEAT-001 P3 — MIMRAI_SSR_SERVER_URL half-rename fix", () => {
	...
	const MIMRAI_SSR_RE = /(?<![A-Z0-9_])MIMRAI_SSR_SERVER_URL(?![A-Z0-9_])/;

	test("auth-client.ts uses NEXUS_SSR_SERVER_URL (not MIMRAI_SSR_SERVER_URL)", () => {
	...
	test("trpc.ts uses NEXUS_SSR_SERVER_URL (not MIMRAI_SSR_SERVER_URL)", () => {
...
describe("FEAT-001 P3 — Better Auth cookie session no-regression", () => {
	test("auth.ts still calls auth.api.getSession (cookie-session path preserved)", () => {
		const src = readSource(AUTH_MIDDLEWARE_PATH);
		expect(
	...
	test("auth.ts imports Session type from better-auth (Better Auth types retained)", () => {
		const src = readSource(AUTH_MIDDLEWARE_PATH);
		expect(
	...
	test("auth-client.ts still creates a Better Auth client with cookie credentials (cookie flow intact)", () => {
		const src = readSource(AUTH_CLIENT_PATH);
