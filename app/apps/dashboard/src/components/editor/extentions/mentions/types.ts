...
/**
 * Knowledge note entity for mentions
 */
export interface KnowledgeMentionEntity extends MentionableEntity {
	type: "knowledge";
	name: string;
...
/**
 * Prompt entity for mentions.
 * `parentSlug` encodes "productSlug:promptSlug" so the mention can route to
 * /prompts/[productSlug]/[promptSlug] without an extra lookup — matches the
 * iter3 global-search prompt result shape.
 */
export interface PromptMentionEntity extends MentionableEntity {
	type: "prompt";
	name: string;
