...
- **Vercel AI SDK 4** via `@ai-sdk/anthropic`. Azure-routed; baseURL from `AI_API_BASE_URL`. Model `Codex-sonnet-4-6`. No direct Anthropic SDK calls.
...
`ingestion/`, `models/`, `docker-compose*.yml`, `Caddyfile`, `.memory/`, `.Codex/`, anywhere outside the repo.
