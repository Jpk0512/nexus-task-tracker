...
- `ARIZE_PROJECT_NAME` is set in `.Codex/settings.json env` — read from there; do not hardcode.
...
Business logic in `app/` outside `api/auth/` and `api/mcp/`. Business logic in `ingestion/` outside `src/auth/` and `src/clients/`. `models/`. `.env`, `.env.dev`, `.env.prod`. `.memory/`. `.Codex/`. Anywhere outside the repo.
