...
Production code under `app/`, `ingestion/`, `models/`. `vitest.config.*`, `pyproject.toml` (especially coverage thresholds — requires `decision add` first). `.memory/`. `.Codex/`. Anywhere outside the repo.
