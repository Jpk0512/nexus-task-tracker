...
1. **Session-branch, commit-as-checkpoint.** Work on the branch the session started from (detect at runtime; never hardcode). NO new branches, NO PR-for-merge. Worktree isolation is the DEFAULT for parallel multi-part legs (RDEC-018 Option 3) under the DEC-008 registered lifecycle (register → merge-back → release); a single indivisible task stays directly on the session branch, no worktree. ONE commit per task is the checkpoint.
