...
**Worktree isolation is the DEFAULT for parallel multi-part implementation work (RDEC-018 Option 3).** When `>=2` independent code-writing legs are dispatched, each leg registers and runs in its own worktree on branch `feat/<slug>` under the DEC-008 self-managed lifecycle. A SINGLE indivisible workflow stays directly on the session branch and does NOT use a worktree; read-only personas (Scout/Lens) always share the tree.

- **Registered lifecycle, not a bare `git worktree add`.** A worktree is permitted ONLY when the owning workflow registers ownership with the broker `worktree_registry` BEFORE `git worktree add` runs, and auto-merge-back-to-the-session-branch + auto-removal of the worktree/branch (and de-registration) is a **MANDATORY final phase** — **no orphan worktree or branch may survive a completed teammate.** A bare/unregistered `git worktree add` remains DENIED by the worktree guard. If a leg cannot register ownership or guarantee the auto-merge-back+remove step, it MUST NOT use a worktree — fall back to disjoint-file partitioning on the session branch.

  > *Advisory (DEC-008/RDEC-018):* the registered-worktree path is the normal way parallel code-writing legs isolate truly conflicting writes — not a rare escape hatch. The guarantee is unchanged: auto-merge-back + auto-delete the worktree/branch as a mandatory final phase. No hook detects an orphaned worktree after creation — this remains an orchestrator self-discipline rule (tracked as a code follow-up).
...
Nexus personas develop **directly on the branch the session was created from** — the current/active branch at session start. The working branch is **dynamic, detected at runtime** via `git branch --show-current`; it MAY be `main` OR any other branch (some projects are worked off a non-main branch). The working branch is NEVER hardcoded. There are **NO** new per-task feature branches and **NO** pull-request-for-merge ceremony. A registered DEC-008 worktree per leg is the DEFAULT isolation for parallel multi-part work (Article XIII.c, RDEC-018 Option 3); a SINGLE indivisible task stays directly on the session branch with no worktree.

- **Commit-as-checkpoint.** ONE focused commit per completed task IS the checkpoint. Every commit is revertable, so divergent history is unnecessary — to undo a task, revert its commit. There is no new branch to merge and nothing to clean up (a registered worktree's merge-back-and-remove is its own mandatory final phase, per Article XIII.c).
- **Who pushes.** Only the **orchestrator or the user** pushes the session branch. A sub-agent COMMITS on the session branch (or its registered worktree) but does NOT push it — it commits and lets the orchestrator (or the user) push. An explicitly user-authorized sub-agent push is the only exception, gated by an explicit bypass token.
- **Parallel personas.** `>=2` independent code-writing legs register a worktree per leg (Article XIII.c); a single leg or a sequential write-dependency chain coordinates directly on the session branch. Read-only personas (Scout, Lens) share the same tree.
...
**Violation:** Creating a new per-task feature branch, an unregistered/bare `git worktree add`, or a registered worktree left orphaned (no auto-merge-back-and-remove), pushing the session branch from a sub-agent without an explicit user-authorized bypass token, or deploying without the human deploy-step handoff — each is a contract violation. The orchestrator must remove the orphan branch/worktree, restore work to the session branch, and re-route the deploy decision through the human handoff.
