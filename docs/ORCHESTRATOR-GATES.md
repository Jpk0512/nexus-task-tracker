...
> commit is revertable), there are no per-task feature branches, and no pull-request-for-merge
> ceremony. A registered DEC-008 worktree per leg is the DEFAULT isolation for parallel multi-part
> work (RDEC-018 Option 3); a single indivisible task stays directly on the session branch with no
> worktree. A sub-agent **commits** on the
...
|---|---|---|---|---|---|
| **broker-gate.py** | `PreToolUse` matcher `Task\|TeamCreate` — fires before every Task/TeamCreate dispatch | **F1-04 TOKEN-AUTHORITATIVE (default).** Reads `.memory/files/broker_state.json`; requires `state.capability_token` (minted by `nexus_validate_brief` on plan-gate PASS) to verify fail-closed — schema/alg/signature/kid/jti-denylist/expiry all pass, TTL default 4h (`NEXUS_TOKEN_TTL_SECONDS`). The token's own `persona` claim IS the persona binding. No per-dispatch `called_at`/`notepad_logged_at` freshness check runs in this mode. **Rollback (`NEXUS_RITUAL_AUTHORITY=1`, kept 1 release):** restores the pre-F1-04 predicate verbatim — `approved == true` AND `called_at` ≤ `TURN_STALE_SECONDS=300` (DEC-068) old; standard/complex tiers additionally require `notepad_logged_at` within `NOTEPAD_STALE_SECONDS=900s` (DEC-068). Resolves repo root by walking up for `.memory/`; honors `NEXUS_BROKER_STATE_PATH` override | **DENY/exit2** (FAIL-CLOSED, both modes). A missing/malformed/unreadable `broker_state.json` → exit 2. Set `NEXUS_BROKER_ALLOW_DEGRADED=1` to override: allowed (exit 0) but a LOUD `additionalContext` warning every turn | Token mode: `no valid capability token for this dispatch (reason=<absent\|tampered\|expired\|unknown-kid\|...>). Call nexus_validate_brief to mint a fresh capability token for this persona/plan, or set NEXUS_RITUAL_AUTHORITY=1 to roll back to the pre-F1-04 validate/notepad ritual.`; `capability token was minted for persona '<X>' but this dispatch targets '<Y>' (persona-mismatch)...`. Ritual mode (rollback): `broker rejected dispatch to '<persona>' — Task dispatch not allowed. Call nexus_validate_brief with a valid brief first.`; `broker_state.json has no called_at timestamp — nexus_validate_brief was not called this turn.`; `broker_state.json is stale (<n>s old, max 300s) — call nexus_validate_brief again for this turn.`; `notepad_logged_at is absent — run 'python3 .memory/log.py notepad list --topic <scope>' and call nexus_notepad_ping before dispatching.`; `notepad_logged_at is stale (<n>s old, max 900s) — re-run the notepad ritual and nexus_notepad_ping for this turn.`; `<file> — broker unavailable. Start nexus-broker or set NEXUS_BROKER_ALLOW_DEGRADED=1 to bypass.` | Token mode (default): call `nexus_validate_brief` (`mcp__nexus-broker__nexus_validate_brief_tool`) once per task/plan — it mints the token — then dispatch the Task to the SAME persona; no notepad-ping repetition required by this gate. Ritual mode (`NEXUS_RITUAL_AUTHORITY=1`): call `nexus_validate_brief` **this turn** (writes `approved` + `called_at`), then (for standard/complex tiers) **MANDATORY** `mcp__nexus-broker__nexus_notepad_ping` (writes `notepad_logged_at`; broker blocks if absent or >900s stale); re-issue the Task within 300s. **TRANSITION:** a broker process mints tokens only after restart/MCP-reconnect — set `NEXUS_RITUAL_AUTHORITY=1` until that is confirmed. Full ritual: `docs/NEXUS-OPERATING-MANUAL.md` §3 |
...
| **worktree-guard** | `PreToolUse` matcher `Bash` | Matches `git worktree add` at command position (heredoc/subshell/wrapper-aware); separately detects creation of a NEW divergent branch (`git checkout -b`, `git switch -c`, `git branch <new>`) off the session branch | **ALLOW/exit0** on `git worktree add` when a LIVE registered-ownership record exists (`nexus_register_worktree`) — registered worktrees are the DEFAULT isolation for parallel multi-part legs (RDEC-018 Option 3). **DENY/exit2** on a bare/unregistered `git worktree add`, and on new-branch creation — Article XIV. User-approval bypass token for branches, under the same merge-back-and-remove obligation | Nested `permissionDecision`, typed codes `WORKTREE/ADD-ALLOWED` / `WORKTREE_DENIED` / `NEW_BRANCH_DENIED` / `NEW_BRANCH_BYPASS`. E.g. `[worktree-guard] WORKTREE ALLOWED — registry ownership verified...` / `WORKTREE_DENIED — git worktree add has no live registry record...` | At worktree-add: register the path first via `nexus_register_worktree` (owner_id + ttl_seconds), then retry; auto-merge-back-and-remove is a mandatory final phase. At new-branch denial: commit on the session branch; use `# BYPASS:USER-APPROVED-BRANCH` only when user approved AND merge-back + delete on completion is guaranteed |
...
**broker dispatch gate** (every Task), the **push-identity gate + worktree-guard** (who may push /
registered-worktree-only), and the **deploy-step human handoff** (the single human release checkpoint). The deploy-step
...
|---|---|---|
| **DEC-008 worktree cleanup** | CLAUDE.md + Constitution DEC-008 / RDEC-018 | Registered worktrees are the DEFAULT isolation for parallel multi-part legs; auto-merge-back AND worktree/branch removal MUST happen as a mandatory final phase. No hook detects or blocks an orphaned worktree. |
...
| `forge-wire` | `implement_ui`, `implement_api` | Server-side API / server action implementer. Split from retired base `forge`. Escalation is a `tier=pro` (`model: opus, effort: xhigh`) parameter on this SAME persona, not a separate name. |
| `forge-ui` | `implement_ui`, `implement_api` | RSC / React / Tremor UI implementer. Split from retired base `forge`. Escalation is a `tier=pro` (`model: opus, effort: xhigh`) parameter on this SAME persona, not a separate name. |
| `pipeline-data` | `implement_ingestion` | Polars/DuckDB data-transform implementer. Split from retired base `pipeline`. Escalation is a `tier=pro` (`model: opus, effort: xhigh`) parameter on this SAME persona, not a separate name. |
| `pipeline-async` | `implement_ingestion` | Dramatiq/Redis async-worker implementer. Split from retired base `pipeline`. Escalation is a `tier=pro` (`model: opus, effort: xhigh`) parameter on this SAME persona, not a separate name. |
...
**Retired `-pro` escalation NAMES.** `forge-ui-pro`, `forge-wire-pro`,
`pipeline-data-pro`, `pipeline-async-pro` are ALSO retired as dispatch targets
(R2-T03 FIX-4): each base/pro pair collapsed to one tier-parameterized source
(`tier=base|pro`), so escalation is expressed as a parameter on the base persona,
never as a separate persona name. A dispatch to a retired `-pro` name is
REJECTED by the broker exactly as `persona-alias-resolver.sh` rejects it — see
`nexus-broker/tests/test_pro_variant_retirement.py` for the enforced agreement.
No agent file exists for any of the four names; their `deliverables.json` rows
are a boundary compat shim only (catch a stray dispatch attempt), not a
registered dispatch target.

**Classifier vs orchestrator-only.** All personas above are classifier-emittable *except* `lens-fast` (`NON_CLASSIFIER_PERSONAS` in `registry.py`). The retired `-pro` names are NOT classifier-emittable — the router selects `tier=pro` as a parameter on the base persona, never the retired name.
